...
class Bootstrap extends CApplicationComponent
{
    ...
    /**
     * @var bool enable use CDN servers for bootstrap CSS/JS
     * @since YiiBooster 1.0.7
     */
    public $enableCDN = true;

    public static $cdn = array(
        'css' => array(
            'bootstrap.css'            => '//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.1/css/bootstrap.min.css',
            'bootstrap-responsive.css' => '//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.1/css/bootstrap-responsive.min.css',
            'font-awesome.css'         => '//netdna.bootstrapcdn.com/font-awesome/3.0.2/css/font-awesome.min.css',
            'bootstrap-combined.css'   => '//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.1/css/bootstrap-combined.min.css',
        ),
        'js'  => array(
            'bootstrap.js' => '//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.1/js/bootstrap.min.js',
            'moment.js'    => '//cdnjs.cloudflare.com/ajax/libs/moment.js/2.0.0/moment.min.js',
            'date.js'      => '//cdnjs.cloudflare.com/ajax/libs/datejs/1.0/date.min.js',
            'spin.js'      => '//cdnjs.cloudflare.com/ajax/libs/spin.js/1.2.7/spin.min.js',
            'select2.js'   => '//cdnjs.cloudflare.com/ajax/libs/select2/3.3.1/select2.min.js',
            'wysihtml5.js' => '//cdnjs.cloudflare.com/ajax/libs/wysihtml5/0.3.0/wysihtml5.min.js',
        )
    );
	...
	{
        if ($this->enableCDN && isset(self::$cdn['css'][str_replace('.min', '', $cssFile)]))
            Yii::app()->getClientScript()->registerCssFile(self::$cdn['css'][str_replace('.min', '', $cssFile)]);
        else
	...
	public function registerJS($position = CClientScript::POS_HEAD)
	{
		...
		if ($this->enableBootboxJS)
            $this->registerAssetJs('bootstrap.bootbox.min.js', $position);

		/* enable bootstrap notifier ? */
		if ($this->enableNotifierJS)
		{
            $this->registerAssetCss('bootstrap-notify.css', $position);
            $this->registerAssetJs('bootstrap.notify.js', $position);
		}
        $this->registerAssetJs('bootstrap' . (!YII_DEBUG ? '.min' : '') . '.js', $position);
	}
	...
	{
        if ($this->enableCDN && isset(self::$cdn['js'][str_replace('.min', '', $jsFile)]))
            Yii::app()->getClientScript()->registerScriptFile(self::$cdn['js'][str_replace('.min', '', $jsFile)]);
        else
