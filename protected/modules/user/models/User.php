...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		return array(
			...
			array(
				...
				'message' => Yii::t('user', 'Неверный формат поля "{attribute}" допустимы только буквы и цифры, от 2 до 20 символов')
			),
			array('email', 'email'),
	...
	public function getAvatar($size = 64, $htmlOptions = array())
	{
		...
		{
			return CHtml::image('http://gravatar.com/avatar/' . md5($this->email) . "?d=mm&s=" . $size, $this->username, $htmlOptions);
		}
		else
		{
			return '';
	...
	public function assignAdminRole($userId)
	{
		...
		Yii::app()->db->createCommand("INSERT INTO {$assignmentTable} (`itemname`,`userid`,`bizrule`,`data`)
                									VALUES ('Admin','{$userId}', NULL, 'N;')")->execute();
	}

	public function revokeAdminRole($userId)
	{
		$assignmentTable = Yii::app()->getAuthManager()->assignmentTable;
		Yii::app()->db->createCommand("DELETE FROM {$assignmentTable} WHERE `userid`={$userId} AND `itemname`='Admin'")->execute();
	}
