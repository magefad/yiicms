...
$this->menu = array(
	...
	array('icon' => 'list-alt white', 'label' => Yii::t('menu', 'Управление'), 'url' => array('menuItem/admin')),
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('menuitem-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<?php $this->widget('CustomTbGridView',array(
	...
	'columns'	=> array(
		...
		array(
			...
			#'value' => 'MenuItem::model()->findByPk($data->parent_id)->title',
			#'value' => '$data->parent_id ? MenuItem::model()->findByPk($data->parent_id)->title : Yii::t("menu", "нет")',
			#'filter' => array_merge(array(0 => 'Корень'), CHtml::listData(MenuItem::model()->findAll(array('order'=>'title')), 'id', 'title')),
		),
		/*array(
			'name' => 'condition_name',
		...
		array(
			...
			'filter' => array('' => Yii::t('menu', 'Все'), '1' => Yii::t('menu', 'Включено'), 0 => Yii::t('menu', 'Выключено')),
